...
double multilinear_interpolation (uint64_t n, double * target, double (* inputs) [2], double * outputs) {
    ...
    for (uint64_t i = 0; i < m; i ++) {
        ...
        for (uint64_t j = 0; j < n; j ++) {
            if (i >> j & 1)
                product *= target [j] - inputs [j] [0];
            else
                product *= inputs [j] [1] - target [j];
        }
        sum += product;
    }
    product = 1;
    for (uint64_t i = 0; i < n; i ++)
        product *= inputs [i] [1] - inputs [i] [0];
    return sum / product;
}
...
void inv_perspective_proj_mat4 (double aspect_ratio, double fov_v, double near, double far, void * mat) {
    double (* mat_) [4] = reinterpret_cast <double (*) [4]> (mat);
    memset (mat, 0, 16 * sizeof (double));
    mat_ [2] [2] = tan (fov_v * .5);
    mat_ [0] [0] = mat_ [2] [2] * aspect_ratio;
    mat_ [3] [3] = 1 / near;
    mat_ [3] [1] = - (far - near) * mat_ [3] [3] / far;
    mat_ [1] [3] = 1;
}
...
void camera_euler_zyx_mat4 (double * position, double * rotation, void * mat) {
    ...
    double cos_values [3] = { cos (rotation [0]), cos (rotation [1]), cos (rotation [2]) };
    double sin_values [3] = { sin (rotation [0]), sin (rotation [1]), sin (rotation [2]) };
    mat_ [0] [0] = cos_values [1] * cos_values [2];
    mat_ [0] [1] = cos_values [1] * sin_values [2];
    mat_ [0] [2] = - sin_values [1];
    mat_ [0] [3] = - vec_dot_prod (position, mat_ [0], 3);
    mat_ [1] [0] = sin_values [0] * sin_values [1] * cos_values [2] - cos_values [0] * sin_values [2];
    mat_ [1] [1] = sin_values [0] * sin_values [1] * sin_values [2] + cos_values [0] * cos_values [2];
    mat_ [1] [2] = sin_values [0] * cos_values [1];
    mat_ [1] [3] = - vec_dot_prod (position, mat_ [1], 3);
    mat_ [2] [0] = cos_values [0] * sin_values [1] * cos_values [2] + sin_values [0] * sin_values [2];
    mat_ [2] [1] = cos_values [0] * sin_values [1] * sin_values [2] - sin_values [0] * cos_values [2];
    mat_ [2] [2] = cos_values [0] * cos_values [1];
    mat_ [2] [3] = - vec_dot_prod (position, mat_ [2], 3);
    mat_ [3] [0] = 0;
    mat_ [3] [1] = 0;
    mat_ [3] [2] = 0;
...
void mat4_to_euler_zyx (void * mat, double * rotation) {
    ...
    if (mat_ [2] [0] == 1) {
        rotation [0] = 0;
        rotation [1] = M_PI_2;
        rotation [2] = - atan2 (mat_ [0] [1], mat_ [0] [2]);
    } else if (mat_ [2] [0] == - 1) {
        rotation [0] = 0;
        rotation [1] = - M_PI_2;
        rotation [2] = atan2 (- mat_ [0] [1], - mat_ [0] [2]);
    } else {
        rotation [0] = atan2 (mat_ [2] [1], mat_ [2] [2]);
        rotation [1] = - asin (mat_ [2] [0]);
        rotation [2] = atan2 (mat_ [1] [0], mat_ [0] [0]);
    }
}
